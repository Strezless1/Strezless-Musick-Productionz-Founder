...
var storeOrderCreate = cli.Command{
	Name:    "create",
	...
	Action:          handleStoreOrderCreate,
	HideHelpCommand: true,
}

var storeOrderRetrieve = cli.Command{
	Name:    "retrieve",
	...
	Action:          handleStoreOrderRetrieve,
	HideHelpCommand: true,
}

var storeOrderDelete = cli.Command{
	Name:    "delete",
	...
	Action:          handleStoreOrderDelete,
	HideHelpCommand: true,
}

func handleStoreOrderCreate(ctx context.Context, cmd *cli.Command) error {
	...
	params := strezlessmusicknexusmetadata.StoreOrderNewParams{}
	...
	_, err = client.Store.Order.New(ctx, params, options...)
	if err != nil {
		return err
	}
...
func handleStoreOrderRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	var res []byte
	options = append(options, option.WithResponseBodyInto(&res))
	_, err = client.Store.Order.Get(ctx, cmd.Value("order-id").(int64), options...)
	if err != nil {
		return err
	}
...
func handleStoreOrderDelete(ctx context.Context, cmd *cli.Command) error {
	...
	return client.Store.Order.Delete(ctx, cmd.Value("order-id").(int64), options...)
}
